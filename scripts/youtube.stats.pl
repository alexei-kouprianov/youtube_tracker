...
open (SRC00, '<urllist.txt') or die $!;
open (TGT00, '>>../downloads/pages.raw.txt') or die $!;
while(<SRC00>){
	print TGT00 get($_);
	}
close TGT00;
...
open (SRC01, '<../downloads/pages.raw.txt') or die $!;
open (TGT01, '>>../downloads/pages.processed.01.txt') or die $!;
	while (<SRC01>) {
		s/ \- YouTube//g;
		s/(?=\"LIKE\")|(?=\"DISLIKE\")/\n/g;
		print TGT01 $_;
...
open (SRC02, '<../downloads/pages.processed.01.txt') or die $!;
